...
REAL_TAG_WIDTH = 0.05  # meters (example)
FOCAL_LENGTH = 500     # pixels (example)

# Clamping ranges (adjust to keep poses within reachable workspace)
X_MIN, X_MAX = 0.1, 0.4
Y_MIN, Y_MAX = -0.2, 0.2
...
def make_pose(x_m, y_m, z_m, angle_deg=0):
    """
    Build a homogeneous transform that places the tool at (x_m, y_m, z_m) in robot base 
    coordinates with a fixed orientation that is then rotated about Z by the given angle.
    
    The fixed orientation is set by applying a 180° rotation about X so that the tool faces forward.
    Then, a rotation about Z (by angle_deg) is applied to account for the tag's in-plane rotation.
    All translations are in meters (converted to mm).
    """
    T = rdm.transl(x_m*1000, y_m*1000, z_m*1000)
    T *= rdm.rotx(math.radians(180))          # fixed flip (adjust if needed)
    T *= rdm.rotz(math.radians(angle_deg))      # additional rotation about Z
    return T
...
class AprilTagApp(QMainWindow):
    def __init__(self):
        ...
        self.move_thread = None  # thread for asynchronous robot movement

        # --- RoboDK Connection ---
        self.RDK = rdk.Robolink()
        # IMPORTANT: Make sure the string exactly matches your robot's name in RoboDK.
    ...
    def move_robot_async(self, x, y, z, angle_deg):
        # If a move is already in progress, skip
    ...
    def _move_robot_sync(self, x, y, z, angle_deg):
        """
        Moves the robot asynchronously using a target pose that incorporates a translation
        and a rotation. The rotation is built by applying a 180° rotation about X and then a
        rotation about Z by angle_deg.
        """
        pose_target = make_pose(x, y, z, angle_deg)
        try:
        ...
        time.sleep(1)
        pose_lift = make_pose(x, y, z+0.1, angle_deg)
        try:
    ...
    def update_frame(self):
        ...
        if ids is not None:
            ...
            if now - self.last_move_time > self.move_delay:
                ...
                # Compute the in-plane angle from the top edge (first two corners)
                ...
                # Move robot asynchronously with computed angle
                self.move_robot_async(x_m, y_m, z_m, angle_deg)
