...
def make_pose(x_m, y_m, z_m, angle_deg=0):
    ...
    T *= rdm.rotz(math.radians(angle_deg))
    return T
...
class AprilTagApp(QMainWindow):
    def __init__(self):
        ...
        # IMPORTANT: Ensure the string exactly matches your robot's name in RoboDK.
        ...
        # NEW: Angle label (persistent display of computed angle)
    ...
    def update_frame(self):
        ...
        if ids is not None:
            ...
            if now - self.last_move_time > self.move_delay:
                ...
                # Update the angle label so it is always visible
                self.angle_label.setText(f"Angle: {angle_deg:.1f} deg")
                
                # Convert pixel coordinates to robot coordinates using fixed offset and scale
